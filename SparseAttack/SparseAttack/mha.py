...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(10*128,256) # 10 
...
class MLPEncoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        position_enc = position_encoding(node_size, 5).to(DEVICE)  # (10,5)
        pe = torch.stack([position_enc for _ in range(batch_size)], dim=0).cuda() # (2500, 10, 5)
        # print(f"In SeqNet forward: x with position encoding: {pe.shape}") 
        x = torch.cat([x, pe], dim=-1) # (2500, 10, 5+5) 
        # print(f"In SeqNet forward: x with position encoding: {x.shape} {x}")  
