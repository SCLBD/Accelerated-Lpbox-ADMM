...
def main():
    ...
    gg = open('result/generate_iterations.csv', 'a+')
...
def train_adptive(i, model, images, target, B, noise_Weight, img_file):
    ...
    for search_time in range(1, args.lambda1_search_times+1):
        # f = None 
        f = open(f"xiter/{img_file}_{search_time}.csv", 'w+')
...
def update_G_l2f(model, images, target_label, epsilon, G, init_params, B, noise_Weight, out_iter, f):
    ...
    net = 'mha'
    if net=='mlp':
        load_path = "saved_model/final_1/mlp/checkpoint/MHA_19.cp"
    else:
        load_path = "saved_model/final_1/mha/checkpoint/MHA_19.cp"
