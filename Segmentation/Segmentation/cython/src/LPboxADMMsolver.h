...
class LPboxADMMsolver {
	private:
		...
		SparseMatrix *A_ptr, *C_ptr, *E_ptr;
		...
		// ------------- Image Segmentation: quadratic unconstrained. ------------------
		int m;
		int n;
		_double_t _c;
	...
	public:
		...
		int ADMM_bqp_unconstrained_legacy();  // solve iterations

		int ADMM_bqp_unconstrained_l2f(int iter_start, int iter_end, double* vec, int fix_num);
 
		void ADMM_bqp_unconstrained_init(); // solve init.
		...
		_double_t ADMM_bqp_unconstrained_legacy(int n, const SparseMatrix &_A, const DenseVector &_b, 
				const DenseVector &x0, Solution& sol);
		...
		*/
		int ADMM_cluster_init(int n_, const SparseMatrix &_A, const DenseVector &_b, 
			const DenseVector &x0, int m_, const SparseMatrix &_C, const DenseVector &_d, Solution& sol);
